...
conda create --name mononerd python=3.7 pytorch==1.6.0 torchvision==0.7.0 cudatoolkit=10.1 -c pytorch
conda activate mononerd
...
cd mononerd_det
# install required packages by mononerd
pip install -r requirements.txt
# install mononerd in debug mode
python setup.py develop
