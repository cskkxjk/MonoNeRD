...
def update_cfg_by_args(cfg, args):
    ...
    if tags is None:
        tags = ""
    if args.trainval:
        cfg.DATA_CONFIG.DATA_SPLIT.train = 'trainval'
        cfg.DATA_CONFIG.DATA_SPLIT.test = 'test'
        cfg.DATA_CONFIG.INFO_PATH.train = ['kitti_infos_trainval.pkl']
        cfg.DATA_CONFIG.INFO_PATH.test = ['kitti_infos_test.pkl']
        assert 'trainval' in tags
