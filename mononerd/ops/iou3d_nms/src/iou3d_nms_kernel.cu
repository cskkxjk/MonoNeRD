...
__global__ void boxes_overlap_onebyone_kernel(const int num_a, const float *boxes_a, const float *boxes_b, float *ans_overlap){
    // params boxes_a: (N, 7) [x, y, z, dx, dy, dz, heading]
    // params boxes_b: (M, 7) [x, y, z, dx, dy, dz, heading]
...
__global__ void boxes_iou_bev_onebyone_kernel(const int num_a, const float *boxes_a, const float *boxes_b, float *ans_iou){
    // params boxes_a: (N, 7) [x, y, z, dx, dy, dz, heading]
    // params boxes_b: (M, 7) [x, y, z, dx, dy, dz, heading]
    const int idx = blockIdx.x * THREADS_PER_BLOCK + threadIdx.x;
...
void boxesoverlapOnebyoneLauncher(const int num_a, const float *boxes_a, const float *boxes_b, float *ans_overlap){
    ...
    boxes_overlap_onebyone_kernel<<<blocks, threads>>>(num_a, boxes_a, boxes_b, ans_overlap);
#ifdef DEBUG
    cudaDeviceSynchronize();  // for using printf in kernel function
#endif
}
...
void boxesioubevOnebyoneLauncher(const int num_a, const float *boxes_a, const float *boxes_b, float *ans_iou){
    ...
    boxes_iou_bev_onebyone_kernel<<<blocks, threads>>>(num_a, boxes_a, boxes_b, ans_iou);
#ifdef DEBUG
    cudaDeviceSynchronize();  // for using printf in kernel function
#endif
}
