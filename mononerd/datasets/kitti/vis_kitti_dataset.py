# Visualize KITTI Pytorch Dataset (for visualize Our MonoNeRD model, only image and calib)
import os
import copy
import pickle
import numpy as np
...
from mononerd.datasets.stereo_dataset_template import StereoDatasetTemplate
    ...
    def include_kitti_data(self, mode):
        ...
        for _, _, files in os.walk(image_path):
            for file in files:
                ...
                image_info = {'image_idx': stem, 'image_shape': np.array(io.imread(image_file).shape[:2], dtype=np.int32)}
