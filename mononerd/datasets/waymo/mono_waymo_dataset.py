# Monocular Waymo Pytorch Dataset (for training Our MonoNeRD model)
...
from mononerd.utils import box_utils, calibration_kitti, common_utils, object3d_kitti, depth_map_utils
from mononerd.datasets.mono_dataset_template import MonoDatasetTemplate


class MonoWaymoDataset(MonoDatasetTemplate):
    ...
    def include_waymo_data(self, mode):
        ...
        assert len(self.sample_id_list) == len(self.waymo_infos)

        if self.logger is not None:
            self.logger.info('Total samples for Waymo dataset: %d' %
                             (len(waymo_infos)))
    ...
    def __getitem__(self, index):
        ...
        # load images
        left_img = self.get_image(info['image']['image_idx'], 2)[:, :, :3]

        if self.dataset_cfg.RESIZE_RATE:
            left_img = transform.resize(left_img, (int(left_img.shape[0] * self.dataset_cfg.RESIZE_RATE), int(left_img.shape[1] * self.dataset_cfg.RESIZE_RATE)), preserve_range=True)
            calib.P2[:2, :3] = calib.P2[:2, :3] * self.dataset_cfg.RESIZE_RATE
